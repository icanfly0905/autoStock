...
slack = Slacker('xoxb-1595801621813-1599117508115-V9Tbag19udEVMroFNPih1WS2')
def dbgout(message):
...
def get_movingaverage(code, window):
    ...
    try:
        time_now = datetime.now()
        str_today = time_now.strftime('%Y%m%d')
        ohlc = get_ohlc(code, 240) # 240일선 기준으로 가져온다.
        if str_today == str(ohlc.iloc[0].name):
...
def buy_etf(code):
    ...
    try:
        ...
        # 240일 이동평균가격
        ma240_price = get_movingaverage(code, 240)
        ...
        # condition3
        if current_price >= ma240_price :
            # 매수종목::현재 가격
            printlog(stock_name + '::' + current_price)
            ##########
            # 매수로직 #
            ##########
